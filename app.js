...
const PROFILE_FRAMES = {
  ...
  ninjaStarsRGB: { label: "Ninjastjärnor (RGB)", type: "ninjaStarsRGB" },
...
const PROFILE_FRAME_UNLOCK_LEVEL = {
  cometGold: 5, cometGreen: 10, pulseRed: 15, cometPurple: 20, cometPink: 25,
  ...
  eldringJakt: 90, frostringJakt: 95, eldFrostCombo: 100, pinball: 105, pinballComet: 110, pinballDuo: 115, ninjaStars3: 120, ninjaStarsRGB: 125, allaMinaRamar: 130,
};
...
function profileFrameWrapStyle(frameKey, padding, shape) {
  ...
  if (f.type === "ninjaStars3") return { className: "avatar-frame-pinball" + shapeClass, style: `${base};border:2px solid ${f.color};box-shadow:0 0 8px ${hexToRgba(f.color, 0.4)}` };
...
function profileAvatarHTML(size, padding) {
  ...
  const pinballExtra = pinballExtraForFrame(frameKey);
  return `<div class="${frame.className}" style="${frame.style}"${cycleAttrs}${pinballExtra.attrs}><img src="${profile.avatar}" alt="Profilbild" style="width:${size}px;height:${size}px;border-radius:50%;object-fit:cover;display:block" />${pinballExtra.html}</div>`;
}
// Delad hjälpfunktion för alla tre flipper-varianter (enkel boll,
// komet-svans, tvåbollskrock) - används både av profileAvatarHTML och av
// "Alla animationer"-cykeln, så innehållet alltid byggs likadant oavsett var
// ramen dyker upp.
...
const FRAME_EFFECT_KEYS = [
  ...
  "pinball", "pinballComet", "pinballDuo", "ninjaStars3", "ninjaStarsRGB", "allaMinaRamar",
];
...
function initCycleAllFrames() {
  ...
  document.querySelectorAll("[data-cycle-all]").forEach((el) => {
    ...
    const applyFrame = () => {
      ...
      const wasPinball = el.hasAttribute("data-pinball-type");
      const extra = pinballExtraForFrame(key);
      if (extra.attrs) {
...
// --- Flipper-ramerna (tre varianter) -----------------------------------
// 1) "single": en boll som studsar INUTI cirkeln. Banan är en stjärnpolygon
//    ({N/K}) - N punkter jämnt utspridda på en cirkel, sammankopplade i räta
//    linjer var K:e punkt. En sådan stjärna har exakt samma infalls- som
//    utfallsvinkel i varje hörn helt automatiskt tack vare symmetrin -
//    matematiskt en riktig biljard-bana, inte en gissad kurva.
// 2) "comet": samma bana som (1), fast ritad med en tonande svans bakom
//    huvudet istället för en enkel prick.
// 3) "duo": TVÅ bollar som även kan krocka med VARANDRA (inte bara mot
//    kanten) - riktiga elastiska krockar går inte att uttrycka som en enkel
//    upprepande formel som (1)/(2), så den banan är istället en riktig
//    fysiksimulering kör EN gång i förväg (Python, se chatten) och sparad
//    som en lista av positioner (var 40:e ms, ~14s lång slinga) som spelas
//    upp cykliskt - exakt samma "räkna fram från verklig klocktid"-princip
//    som resten av ramarna, bara med en tabell istället för en formel.
const PINBALL_N = 7;
const PINBALL_STEP = 3;
const PINBALL_RADIUS_PCT = 38;
const PINBALL_SEGMENT_MS = 480;
...
const PINBALL_DUO_INTERVAL_MS = 40;
function pinballDuoPositionAtTime(ms) {
  const totalSamples = PINBALL_DUO_SAMPLES.length / 4;
  const cycleMs = totalSamples * PINBALL_DUO_INTERVAL_MS;
  const t = ((ms % cycleMs) + cycleMs) % cycleMs;
  const idx = Math.floor(t / PINBALL_DUO_INTERVAL_MS);
  const frac = (t % PINBALL_DUO_INTERVAL_MS) / PINBALL_DUO_INTERVAL_MS;
  const i0 = idx * 4;
  const i1 = ((idx + 1) % totalSamples) * 4;
  const lerp = (a, b) => a + (b - a) * frac;
  return {
    ax: lerp(PINBALL_DUO_SAMPLES[i0], PINBALL_DUO_SAMPLES[i1]),
    ay: lerp(PINBALL_DUO_SAMPLES[i0 + 1], PINBALL_DUO_SAMPLES[i1 + 1]),
    bx: lerp(PINBALL_DUO_SAMPLES[i0 + 2], PINBALL_DUO_SAMPLES[i1 + 2]),
    by: lerp(PINBALL_DUO_SAMPLES[i0 + 3], PINBALL_DUO_SAMPLES[i1 + 3]),
  };
}
// Bälte-badgen är åttkantig, inte rund - stjärnpolygon-tricket funkar bara
// mot en cirkel (reflektionen är bara garanterat likadan i varje hörn tack
// vare cirkelns symmetri). För åttkanten räknas studspunkterna istället fram
// EXAKT geometriskt (samma princip som riktig biljard-fysik: hitta vilken
// sida strålen tr\u00e4ffar f\u00f6rst, spegla riktningen mot just den sidans normal,
// forts\u00e4tt) - gjort en g\u00e5ng i f\u00f6rv\u00e4g i Python mot exakt samma \u00e5ttkantsform
// som CSS-clippet (.frame-shape-octagon) anv\u00e4nder, s\u00e5 banan matchar den
// riktigt synliga formen p\u00e5 pixeln. Varje sekvens har olika l\u00e4ngd s\u00e5 tiden
// per sträcka \u00e4r proportionerlig mot l\u00e4ngden (annars hade bollen sett ut
// att \u00e4ndra fart mellan korta/l\u00e5nga studsar).
const PINBALL_OCTAGON_WAYPOINTS = [50.0, 45.0, 79.721, 82.871, 39.581, 4.756, 92.0, 71.547, 86.546, 78.496, 33.31, 91.225, 18.787, 18.085, 92.0, 71.101, 52.091, 100.0, 8.0, 68.073, 78.995, 16.663, 63.824, 93.061, 19.728, 82.518, 8.0, 67.574, 58.344, 3.426, 18.204, 81.541, 72.426, 12.452, 32.286, 90.568, 86.507, 21.479, 46.367, 99.594, 92.0, 41.45, 66.02, 8.346, 42.684, 2.767, 26.044, 86.567, 92.0, 38.806, 42.444, 2.921, 60.736, 95.041, 8.0, 56.853, 70.777, 11.395, 53.209, 99.866, 42.973, 97.419, 8.0, 52.856, 49.482, 0.0, 92.0, 54.176, 59.112, 96.082, 45.839, 99.256, 28.486, 11.868, 92.0, 57.86, 40.001, 95.514, 58.294, 3.393, 8.0, 39.812];
const PINBALL_OCTAGON_SEGMENT_MS = [326.2, 595.1, 575.3, 59.9, 370.9, 505.3, 612.5, 333.9, 368.9, 594.0, 527.8, 307.2, 128.7, 552.6, 595.1, 595.1, 595.1, 595.1, 595.1, 500.9, 285.2, 162.6, 578.9, 551.8, 414.6, 636.4, 441.2, 525.2, 611.2, 71.3, 383.9, 455.3, 466.7, 361.0, 92.5, 603.7, 531.4, 435.0, 636.4, 420.8];
const PINBALL_OCTAGON_CUMULATIVE_MS = (() => {
  const cum = [0];
  PINBALL_OCTAGON_SEGMENT_MS.forEach((d) => cum.push(cum[cum.length - 1] + d));
  return cum;
})();
function pinballOctagonPositionAtTime(ms) {
  const cycleMs = PINBALL_OCTAGON_CUMULATIVE_MS[PINBALL_OCTAGON_CUMULATIVE_MS.length - 1];
  const t = ((ms % cycleMs) + cycleMs) % cycleMs;
  let segIndex = 0;
  while (segIndex < PINBALL_OCTAGON_SEGMENT_MS.length - 1 && PINBALL_OCTAGON_CUMULATIVE_MS[segIndex + 1] < t) segIndex++;
  const segStart = PINBALL_OCTAGON_CUMULATIVE_MS[segIndex];
  const segT = (t - segStart) / PINBALL_OCTAGON_SEGMENT_MS[segIndex];
  const ax = PINBALL_OCTAGON_WAYPOINTS[segIndex * 2], ay = PINBALL_OCTAGON_WAYPOINTS[segIndex * 2 + 1];
...
function pinballTick() {
  ...
  pinballTracked.forEach((el) => {
    const type = el.getAttribute("data-pinball-type");
    if (!document.contains(el) || !type) { pinballTracked.delete(el); return; }
    ...
    if (type === "single") {
      ...
      if (!a || !b) return;
      const pos = pinballDuoPositionAtTime(now);
      a.style.left = pos.ax + "%";
      a.style.top = pos.ay + "%";
      b.style.left = pos.bx + "%";
      b.style.top = pos.by + "%";
...
function initPinballFrames() {
  document.querySelectorAll("[data-pinball-type]").forEach((el) => {
...
function renderNav() {
  ...
  tabbar.innerHTML = TABS.map((t) => {
    ...
    if (isActive && isEffect) {
      ...
      const pinballExtra = pinballExtraForFrame(color);
      iconHTML = `<div class="${frame.className}" style="width:${outerSize}px;height:${outerSize}px;${frame.style}"${cycleAttrs}${pinballExtra.attrs}><img src="${emblemSrc}" alt="${t.label}" style="width:${innerSize}px;height:${innerSize}px;object-fit:contain;display:block" />${pinballExtra.html}</div>`;
    } else {
      ...
      <div style="margin-top:6px;display:flex;align-items:center;justify-content:center;gap:14px">
        ...
        <div style="position:relative;display:inline-flex">
          ${(() => {
            ...
            const beltPinballExtra = beltRingActive ? pinballExtraForFrame(currentFrameKey, "octagon") : { attrs: "", html: "" };
            if (!belt.image) {
            ...
            return beltFrame
              ? `<div class="${beltFrame.className}" style="${beltFrame.style}"${beltCycleAttrs}${beltPinballExtra.attrs}>${img}${beltPinballExtra.html}</div>`
...
// searchFoodOFF. Officiell öppen data, se
...
// en sida i taget av hela listan (~2500 livsmedel), därav limit=3000 för
// att få hela listan på en gång. Hämtas EN gång per session och cachas, sen
// filtreras det lokalt på namn - snabbt och skonsamt mot deras server efter
// den första hämtningen. Svarsformen är { _meta, _links, livsmedel: [...] }
// (upptäckt genom att faktiskt testa mot servern tidigare).
